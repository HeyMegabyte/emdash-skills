...
triggers:
  task-types: [build, fix]
  file-patterns: ["src/api/*", "src/routes/*", "src/index.ts", "src/server.ts"]
always-load: false
canonical-owner-of:
  - Hono RPC mode and type-safe client generation
  - Error envelope format ({ error, code, details })
  - Rate limiting patterns (tiered per endpoint)
  - Cursor-based pagination
  - OpenAPI spec generation
  - API versioning strategy
  - Request/response schemas (Zod as source of truth)
